...
func (e *Environment) Create() error {
	...
	hostConf := &container.HostConfig{
		...
		// from the Panel.
		Resources: func() container.Resources {
			resources := e.Configuration.Limits().AsContainerResources()
			// Disable resource limits that are known to be problematic on Windows hosts,
			// even when running in a Linux container environment (WSL2).
			if e.osType == "windows" || runtime.GOOS == "windows" {
		...
		SecurityOpt:    e.getSecurityOpts(),
		ReadonlyRootfs: e.osType != "windows",
		CapDrop:        e.getCapDrop(),
		NetworkMode:    networkMode,
		UsernsMode:     e.getUsernsMode(cfg.Docker.UsernsMode),
	}
...
func (e *Environment) convertMounts() []mount.Mount {
	...
	for _, m := range mounts {
		// Skip Linux-specific mounts if we're running in a Windows container environment.
		if e.osType == "windows" && (strings.HasPrefix(m.Target, "/etc/") || strings.HasPrefix(m.Target, "/run/wings")) {
			continue
		}

		target := e.sanitizeMountTarget(m.Target)
		source := e.sanitizeMountSource(m.Source)
		e.log().WithFields(log.Fields{
			"source":    source,
			"target":    target,
			"read_only": m.ReadOnly,
		}).Info("environment/docker: configuring container mount")
...
func (e *Environment) sanitizeMountSource(s string) string {
...
func (e *Environment) getSecurityOpts() []string {
	if e.osType == "windows" {
		return nil
	}
...
func (e *Environment) getCapDrop() []string {
	if e.osType == "windows" {
		return nil
	}
	return []string{
...
func (e *Environment) getUsernsMode(m string) container.UsernsMode {
	if e.osType == "windows" {
		return ""
	}
	return container.UsernsMode(m)
}

func (e *Environment) sanitizeMountTarget(t string) string {
	if e.osType != "windows" {
		return t
	}
	// If it's already a Windows path, leave it.
	if len(t) > 1 && t[1] == ':' {
		return t
	}
	// If it starts with /, convert it to C:/.
