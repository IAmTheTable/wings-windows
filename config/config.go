...
// FromFile reads the configuration from a file on the disk and then sets the
// global configuration instance to that value.
func FromFile(path string) error {
	...
	if err != nil {
		return errors.WrapIf(err, "config: failed to read file")
	}
	if err := yaml.Unmarshal(b, c); err != nil {
		return errors.WrapIf(err, "config: failed to unmarshal yaml")
	}
	platformFixups(c)
	Set(c)
	return nil
...
// ConfigureDirectories ensures that all of the directories required for Wings
// to run are present on the system.
func ConfigureDirectories() error {
	...
	for _, d := range dirs {
		if err := os.MkdirAll(d, 0o700); err != nil {
			return errors.WrapIf(err, "config: failed to create directory")
		}
	}
	if c.Passwd.Enable {
		if err := os.MkdirAll(c.Passwd.Directory, 0o700); err != nil {
			return errors.WrapIf(err, "config: failed to create passwd directory")
		}
	}
	if c.MachineID.Enable {
		if err := os.MkdirAll(c.MachineID.Directory, 0o700); err != nil {
			return errors.WrapIf(err, "config: failed to create machine-id directory")
		}
	}
	return nil
}
